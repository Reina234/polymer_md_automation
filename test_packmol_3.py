...
def test_generate_solvent_box():
    ...
    output_pdb_file = "test_box.pdb"
    output_gro_file = "test_box.gro"
    ...
    try:
        ...
        packmol_input = generate_packmol_input(
            solvent_file, output_pdb_file, num_molecules, box_size
        )

        # Run Packmol to generate the box
        run_packmol(packmol_input)

        # Convert the output PDB file to a GRO file
        convert_pdb_to_gro(output_pdb_file, output_gro_file)

        logger.info(
            f"Test completed successfully. Generated box saved at {output_gro_file}."
        )
