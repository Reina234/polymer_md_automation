...
from config.paths import ACPYPE_PARAMETERIZER_OUTPUT_DIR, ACPYPE_BASE_NAME, TEMPORARY_OUTPUT_DIR
...
class ACPYPEParameterizer(BaseParameterizer):
    ...
    def __init__(self,  metadata_tracker: Optional[MetadataTracker] = None, acpype_molecule_name: str= ACPYPE_BASE_NAME):
        super().__init__(metadata_tracker)
        self.molecule_name = acpype_molecule_name
        self.raw_output_dir = os.path.join(TEMPORARY_OUTPUT_DIR, f"{self.molecule_name}.acpype")

    def parameterize(self, input_file_path: str, output_name: str, 
                     output_base_dir: str = ACPYPE_PARAMETERIZER_OUTPUT_DIR, additional_notes: Optional[str] = None) -> str:
        
        output_dir = os.path.join(output_base_dir, output_name)
        abs_input_file_path, abs_output_dir = self._prepare_directories(input_file_path, output_dir)
        self._run_acpype_command(abs_input_file_path, TEMPORARY_OUTPUT_DIR)
        files_to_move = [f"{self.molecule_name}_GMX.gro", f"{self.molecule_name}_GMX.top", f"{self.molecule_name}_GMX.itp", f"posre_{self.molecule_name}.itp"]
    ...
    def metadata(self, input_file_path: str, output_dir: str, additional_notes: Optional[str]= None) -> Dict:
        """
        Returns metadata about ACPYPE.
        """
        return {"program(s) used": "ACPYPE (GAFF2)",
